...
{
  options.custom.capabilities.embedded-development = {
    enable = lib.mkEnableOption "utilities for embedded development";
  };

  config =
    let
      cfg = config.custom.capabilities.embedded-development;
    in
    lib.mkIf cfg.enable {
      ...
      services.udev.extraRules = lib.mkIf config.custom.users.enable ''
        ...
        # Raspberry Pi Debug Probe (CMSIS-DAP compatible adapter)
